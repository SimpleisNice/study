...
이번 장에서는 상태를 도입함으로써 애플리케이션에 생명을 붙어 넣는다.
...
선택된 프로퍼티에 따라 자동으로 별을 만들어 내는 컴포넌트를 하나 만들자.
```jsx
...
## 6.2 useState 훅
상태를 리액트 함수 컴포넌트에 넣을 때는 훅스라고 부르는 리액트 기능을 사용한다.
...
- 다양한 조건이 있을것다.
- 가장 중요한건 우리가 만든 컴포넌트의 사용자가 미래에 컴포넌트를 어떻게 사용할지 생각해보는 것이다.
```jsx
export default function App() {
  return (
    <StarRating 
...
이런 패턴 중에는 참고(ref)라는 리액트 기능을 사용해 직접 DOM에 접근하는 방법이 포함된다.

리액트에서 참조는 컴포넌트 생명주기 값을 저장하는 객체이다.
...
상태 데이터를 그 데이터가 필요한 컴포넌트에 도달할때까지 프롭 형태로 모든 중간 컴포넌트를 거쳐서 전달하는 과정은 샌프란시스코에서 워싱턴 DC로 기차를 타고 가는것과 같다.

리액트에서 콘텍스트는 데이터를 위한 제트기와 같다

콘텍스트 프로바이더를 만들어서 데이터를 리액트 콘텍스트에 넣을 수 있다.

콘텍스트 프로바이더는 컴포넌트 트리 전체나 트리 일부를 감싸는 리액트 컴포넌트이다.
